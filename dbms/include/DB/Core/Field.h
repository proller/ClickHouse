...
public:
	...
	template <typename T>
	Field(T && rhs, typename std::enable_if<!std::is_same<typename std::decay<T>::type, Field>::value, void>::type * unused = nullptr)
	{
	...
	template <typename T>
	typename std::enable_if<!std::is_same<typename std::decay<T>::type, Field>::value, Field &>::type operator=(T && rhs)
	{
...
private:
	static const size_t storage_size = std::max({ DBMS_MIN_FIELD_SIZE - sizeof(Types::Which),
		sizeof(Null),
		sizeof(UInt64),
		sizeof(Int64),
		sizeof(Float64),
...
inline void readText(Array & x, ReadBuffer & buf)
{
	throw Exception("Cannot read Array.", ErrorCodes::NOT_IMPLEMENTED);
}
inline void readQuoted(Array & x, ReadBuffer & buf)
{
	throw Exception("Cannot read Array.", ErrorCodes::NOT_IMPLEMENTED);
}
...
inline void writeQuoted(const Array & x, WriteBuffer & buf)
{
...
inline void readText(Tuple & x, ReadBuffer & buf)
{
	throw Exception("Cannot read Tuple.", ErrorCodes::NOT_IMPLEMENTED);
}
inline void readQuoted(Tuple & x, ReadBuffer & buf)
{
	throw Exception("Cannot read Tuple.", ErrorCodes::NOT_IMPLEMENTED);
}
...
inline void writeQuoted(const Tuple & x, WriteBuffer & buf)
{

...
protected:
	StorageSetOrJoinBase(const String & path_,
		const String & name_,
...
public:
	static StoragePtr create(const String & path_,
		...
		const ColumnDefaults & column_defaults_)
	{
		return ext::shared_ptr_helper<StorageSet>::make_shared(
			path_, name_, columns_, materialized_columns_, alias_columns_, column_defaults_);
	}

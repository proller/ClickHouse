...
public:
	...
	{
		return "OptimizeQuery_" + database + "_" + table + "_" + partition + "_" + (final ? "final" : "");
	};

	ASTPtr clone() const override
	{
		return std::make_shared<ASTOptimizeQuery>(*this);
	}
...
protected:
	void formatImpl(const FormatSettings & settings, FormatState & state, FormatStateStacked frame) const override
	{
		...
		if (!partition.empty())
			settings.ostr << (settings.hilite ? hilite_keyword : "") << " PARTITION " << (settings.hilite ? hilite_none : "") << partition;

...
struct RoundAgeImpl
{
	...
	static inline ResultType apply(A x)
	{
		return x < 1 ? 0 : (x < 18 ? 17 : (x < 25 ? 18 : (x < 35 ? 25 : (x < 45 ? 35 : 45))));
	}
};
...
private:
	...
	struct Extend<InputType, typename std::enable_if<std::is_same<InputType, Int8>::value || std::is_same<InputType, Int16>::value>::type>
	{
		using Type = Int64;
	};

	template <typename InputType>
	struct Extend<InputType, typename std::enable_if<std::is_same<InputType, UInt8>::value || std::is_same<InputType, UInt16>::value>::type>
	{
		using Type = UInt64;
	};

	template <typename InputType>
	struct Extend<InputType, typename std::enable_if<std::is_integral<InputType>::value && (sizeof(InputType) >= 4)>::type>
	{
...
struct IntegerRoundingComputation<T,
	rounding_mode,
	scale_mode,
	typename std::enable_if<std::is_integral<T>::value && ((scale_mode == PositiveScale) || (scale_mode == ZeroScale))>::type>
{
	using Divisor = int;
...
template <typename T>
struct IntegerRoundingComputation<T, _MM_FROUND_NINT, NegativeScale, typename std::enable_if<std::is_integral<T>::value>::type>
{
...
template <typename T>
struct IntegerRoundingComputation<T, _MM_FROUND_CEIL, NegativeScale, typename std::enable_if<std::is_integral<T>::value>::type>
{
	using Op = FastModulo<T>;
	using Divisor = typename Op::Divisor;
...
template <typename T>
struct IntegerRoundingComputation<T, _MM_FROUND_FLOOR, NegativeScale, typename std::enable_if<std::is_integral<T>::value>::type>
{
	using Op = FastModulo<T>;
	using Divisor = typename Op::Divisor;
...
class FloatRoundingComputation<Float32, rounding_mode, PositiveScale> : public BaseFloatRoundingComputation<Float32>
{
public:
...
template <int rounding_mode>
class FloatRoundingComputation<Float32, rounding_mode, NegativeScale> : public BaseFloatRoundingComputation<Float32>
{
public:
	static inline void prepare(size_t scale, Scale & mm_scale)
	{
...
template <int rounding_mode>
class FloatRoundingComputation<Float32, rounding_mode, ZeroScale> : public BaseFloatRoundingComputation<Float32>
{
public:
	static inline void prepare(size_t scale, Scale & mm_scale)
	{
...
template <int rounding_mode>
class FloatRoundingComputation<Float64, rounding_mode, PositiveScale> : public BaseFloatRoundingComputation<Float64>
{
public:
	static inline void prepare(size_t scale, Scale & mm_scale)
	{
...
template <int rounding_mode>
class FloatRoundingComputation<Float64, rounding_mode, NegativeScale> : public BaseFloatRoundingComputation<Float64>
{
public:
	static inline void prepare(size_t scale, Scale & mm_scale)
	{
...
template <int rounding_mode>
class FloatRoundingComputation<Float64, rounding_mode, ZeroScale> : public BaseFloatRoundingComputation<Float64>
{
public:
	static inline void prepare(size_t scale, Scale & mm_scale)
	{
...
{
	if (mode == _MM_FROUND_NINT)
		return roundf(x);
	if (mode == _MM_FROUND_FLOOR)
		return floorf(x);
	if (mode == _MM_FROUND_CEIL)
		return ceilf(x);
...
class FloatRoundingComputation<T, rounding_mode, PositiveScale> : public BaseFloatRoundingComputation<T>
{
public:
...
template <typename T, int rounding_mode>
class FloatRoundingComputation<T, rounding_mode, NegativeScale> : public BaseFloatRoundingComputation<T>
{
public:
	static inline void compute(const T * __restrict in, const T & scale, T * __restrict out)
	{
...
template <typename T, int rounding_mode>
class FloatRoundingComputation<T, rounding_mode, ZeroScale> : public BaseFloatRoundingComputation<T>
{
public:
...
/** Реализация высокоуровневых функций округления в том случае, когда возвращается нулевое значение.
	  */
template <typename T, int rounding_mode, ScaleMode scale_mode>
struct FunctionRoundingImpl<T, rounding_mode, scale_mode, typename std::enable_if<scale_mode == NullScale>::type>
{
public:
	static inline void apply(const PaddedPODArray<T> & in, size_t scale, typename ColumnVector<T>::Container_t & out)
	{
...
struct ScaleForRightType<T, U, typename std::enable_if<std::is_floating_point<T>::value && std::is_signed<U>::value>::type>
{
...
template <typename T, typename U>
struct ScaleForRightType<T, U, typename std::enable_if<std::is_floating_point<T>::value && std::is_unsigned<U>::value>::type>
{
	static inline bool apply(const ColumnPtr & column, ScaleMode & scale_mode, size_t & scale)
	{
		using PowersOf10 = typename FillArray<std::numeric_limits<T>::digits10 + 1>::result;
...
template <typename T, typename U>
struct ScaleForRightType<T, U, typename std::enable_if<std::is_integral<T>::value && std::is_signed<U>::value>::type>
{
	static inline bool apply(const ColumnPtr & column, ScaleMode & scale_mode, size_t & scale)
	{
		using PowersOf10 = typename FillArray<std::numeric_limits<T>::digits10 + 1>::result;
...
template <typename T, typename U>
struct ScaleForRightType<T, U, typename std::enable_if<std::is_integral<T>::value && std::is_unsigned<U>::value>::type>
{
	static inline bool apply(const ColumnPtr & column, ScaleMode & scale_mode, size_t & scale)
	{
		using ColumnType = ColumnConst<U>;
...
public:
	...
	{
		return std::make_shared<FunctionRounding>();
	}
...
public:
	...
	DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
	{
		...
		if (arguments.size() == 2)
		{
			...
			if (!(checkType<DataTypeUInt8>(type) || checkType<DataTypeUInt16>(type) || checkType<DataTypeUInt32>(type)
			...
			{
				throw Exception("Illegal type in second argument of function " + getName(), ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT);
			}
		}

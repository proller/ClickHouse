...
using is_safe_convervsion = std::integral_constant<bool,
...
using is_any_int_vs_uint = std::integral_constant<bool,
	std::is_integral<TInt>::value && std::is_integral<TUInt>::value && std::is_signed<TInt>::value && std::is_unsigned<TUInt>::value>;
...
using bool_if_double_can_be_used
	= std::enable_if_t<std::is_integral<TAInt>::value && (sizeof(TAInt) <= 4) && std::is_floating_point<TAFloat>::value, bool>;
...
inline bool greaterOp<DB::Float64, DB::UInt64>(DB::Float64 f, DB::UInt64 u)
{
	...
	return (f >= static_cast<DB::Float64>(std::numeric_limits<DB::UInt64>::max())) || (f >= 0 && static_cast<DB::UInt64>(f) > u);
}
...
inline bool greaterOp<DB::UInt64, DB::Float64>(DB::UInt64 u, DB::Float64 f)
{
	...
	return (f < 0) || (f < static_cast<DB::Float64>(std::numeric_limits<DB::UInt64>::max()) && u > static_cast<UInt64>(f));
}

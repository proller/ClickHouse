...
namespace Conditional
{
	namespace
	{
		...
		private:
			static const PaddedPODArray<TType> & initDataArray(const Block & block, const ColumnNumbers & args, const Branch & br)
			{
				size_t index = br.index;
				const ColumnPtr & col = block.safeGetByPosition(args[index]).column;
		...
		{
		public:
			static bool execute(NumericSourcePtr<TResult> & source, const Block & block, const ColumnNumbers & args, const Branch & br)
			{
	...
	private:
		...
		static NumericSources<TResult> createNumericSources(const Block & block, const ColumnNumbers & args, const Branches & branches)
		{

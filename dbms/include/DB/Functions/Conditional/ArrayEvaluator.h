...
namespace Conditional
{
	namespace
	{
		...
		public:
			Chunk(TType * pos_begin_, size_t size_) : pos_begin{ pos_begin_ }, size{ size_ }, is_const{ true }
			{
			}

			Chunk(std::function<TType(size_t)> accessor_, size_t size_) : accessor{ accessor_ }, size{ size_ }, is_const{ false }
			{
			}
		...
		{
		public:
			ArraySource(const PaddedPODArray<TType> & data_, const ColumnArray::Offsets_t & offsets_) : data{ data_ }, offsets{ offsets_ }
			{
			}
			...
			Chunk<TResult> get() const override
			{
				...
				std::function<TResult(size_t)> get_value = [pos_begin](size_t j) { return static_cast<TResult>(*(pos_begin + j)); };
		...
		{
		public:
			ConstArraySource(const Array & array_) : array{ array_ }
			{
			}
		...
		{
		public:
			ArraySink(PaddedPODArray<TResult> & data_, ColumnArray::Offsets_t & offsets_, size_t data_size_, size_t offsets_size_)
			...
			void store(const Chunk<TResult> & chunk)
			{
				...
				if (chunk.isConst())
				{
					...
					memcpySmallAllowReadWriteOverflow15(
						&data[prev_offset], chunk.data(), chunk.getSize() * sizeof(typename Chunk<TResult>::value_type));
				}
				else
				{
					/// Copy from vector.
		...
		{
		public:
			static bool execute(IArraySources<TResult> & sources, const Block & block, const ColumnNumbers & args, const Branch & br)
			{
	...
	private:
		...
		static IArraySources<TResult> createSources(const Block & block, const ColumnNumbers & args, const Branches & branches)
		{
		...
		static ArraySink<TResult> createSink(Block & block, const IArraySources<TResult> & sources, size_t result, size_t row_count)
		{
			...
			return ArraySink<TResult>{ col_res_vec->getData(), col_res_array->getOffsets(), data_size, offsets_size };
		}
	};

...
{
public:
	CacheDictionary(const std::string & name,
	...
	{
		return std::make_unique<CacheDictionary>(*this);
	}
...
#define DECLARE(TYPE) void get##TYPE(const std::string & attribute_name, const PaddedPODArray<Key> & ids, PaddedPODArray<TYPE> & out) const;
	DECLARE(UInt8)
	DECLARE(UInt16)
	DECLARE(UInt32)
	DECLARE(UInt64)
...
#define DECLARE(TYPE)                                                                                                                      \
	void get##TYPE(                                                                                                                        \
		const std::string & attribute_name, const PaddedPODArray<Key> & ids, const PaddedPODArray<TYPE> & def, PaddedPODArray<TYPE> & out) \
		const;
	DECLARE(UInt8)
	DECLARE(UInt16)
	DECLARE(UInt32)
...
#define DECLARE(TYPE)                                                                                                                      \
	void get##TYPE(const std::string & attribute_name, const PaddedPODArray<Key> & ids, const TYPE def, PaddedPODArray<TYPE> & out) const;
	DECLARE(UInt8)
	DECLARE(UInt16)
	DECLARE(UInt32)
	DECLARE(UInt64)
...
private:
	...
	void getItemsNumber(
		Attribute & attribute, const PaddedPODArray<Key> & ids, PaddedPODArray<OutputType> & out, DefaultGetter && get_default) const;

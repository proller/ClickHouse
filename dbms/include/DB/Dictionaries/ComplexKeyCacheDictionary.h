...
private:
	...
	void getItemsNumber(Attribute & attribute,
		const ConstColumnPlainPtrs & key_columns,
		PaddedPODArray<OutputType> & out,
		DefaultGetter && get_default) const;

	template <typename AttributeType, typename OutputType, typename DefaultGetter>
	void getItemsNumberImpl(Attribute & attribute,
		const ConstColumnPlainPtrs & key_columns,
		PaddedPODArray<OutputType> & out,
		DefaultGetter && get_default) const;
	...
	void getItemsString(
		Attribute & attribute, const ConstColumnPlainPtrs & key_columns, ColumnString * out, DefaultGetter && get_default) const;

	template <typename PresentKeyHandler, typename AbsentKeyHandler>
	void update(const ConstColumnPlainPtrs & in_key_columns,
	...
	static StringRef placeKeysInPool(const std::size_t row, const ConstColumnPlainPtrs & key_columns, StringRefs & keys, Arena & pool);

	StringRef placeKeysInFixedSizePool(const std::size_t row, const ConstColumnPlainPtrs & key_columns) const;

...
struct SetMethodOneNumber
{
	...
	struct State
	{
		...
		Key getKey(const ConstColumnPlainPtrs & key_columns, /// Ключевые столбцы.
			...
			const /// Если ключи фиксированной длины - их длины. Не используется в методах по ключам переменной длины.
		{
...
struct SetMethodString
{
	...
	struct State
	{
		...
		Key getKey(const ConstColumnPlainPtrs & key_columns, size_t keys_size, size_t i, const Sizes & key_sizes) const
		{
